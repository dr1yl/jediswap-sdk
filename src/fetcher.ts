...
export abstract class Fetcher {
  ...
  public static async fetchTokenData(
    chainId: ChainId,
    address: string,
    provider = new Provider({ sequencer: { network: NetworkNames[chainId] } }),
    symbol?: string,
    name?: string
  ): Promise<Token> {
  ...
  public static async fetchPairData(
    ...
    const { reserve0, reserve1 } = await new Contract(PairAbi as Abi, address, provider).call('getReserves', [])
